...
export const createFileTool: ToolDefinitionFull<z.infer<typeof schema>> = { name: 'create_file', description: 'Create a new file inside the workspace.', parameters: schema, risk: 'write', async run(input, ctx) { const abs = resolveWorkspacePath(ctx.workspaceRoot, input.path); await fs.mkdir(path.dirname(abs), { recursive: true }); await fs.writeFile(abs, input.content, { flag: 'wx' }); await snapshotFile(ctx.workspaceRoot, ctx.sessionId, input.path); return { ok: true, output: `Created ${input.path}` }; } };
